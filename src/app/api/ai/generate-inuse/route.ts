...
export async function POST(request: Request) {
  try {
    ...
    const { data: configData } = await supabase.from('config').select('value').eq('key', 'googleApiKey').maybeSingle()
    const googleApiKey = typeof configData?.value === 'string' ? configData.value.trim() : ''
    if (!googleApiKey) {
    ...
    const mimeType = mimeTypeForFilename(imageFilename)
    ...
    const generateOne = async () => {
      try {
        ...
        if (image) {
          return { success: true as const, image: image.image, mimeType: image.mimeType }
        }
