...
export default function PortfolioProjectPage() {
  ...
  useEffect(() => {
    fetch(`/api/public/portfolio/${slug}`)
      .then((r) => {
  ...
  if (loading) {
    return (
      <div className="max-w-[1200px] px-4 sm:px-6 md:px-[48px] py-12 mx-auto">
        <div className="h-[40px] w-2/3 bg-[var(--c-gray-100)] rounded animate-pulse mb-6" />
  ...
  if (notFound || !project) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] text-[var(--c-gray-400)] text-[13px] gap-3">
        <p>Project not found</p>
        <Link href="/portfolio" className="text-[var(--c-gray-600)] hover:text-[var(--c-gray-900)] transition-colors">
  ...
  const heroMedia = heroImage
    ? allMedia.find((m) => m.filename === heroImage) ?? null
    : null
    ...
    <>
      <div className="bg-[var(--c-black)] text-white">
        <div className="sticky top-0 z-40 bg-[var(--c-black)]/80 backdrop-blur-xl">
          <div className="max-w-[1040px] px-4 sm:px-6 md:px-[48px] py-3">
            <Link href="/portfolio" className="text-[12px] font-[400] text-white/30 hover:text-white/70 transition-colors duration-200">
              &larr; Portfolio
            </Link>
          </div>
        </div>
