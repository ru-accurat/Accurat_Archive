...
export function IndexPageClient({ initialProjects }: Props) {
  const router = useRouter()
  const { viewMode, setViewMode, editMode, setEditMode } = useUiStore()
  const { setSearch, setProjects, filters, projects } = useProjectStore()
  const filtered = useFilteredProjects()

  // Hydrate the store with server-fetched data on first mount.
  // Subsequent navigations use the cached store data.
  useEffect(() => {
