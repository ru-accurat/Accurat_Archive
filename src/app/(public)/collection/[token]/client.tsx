...
export function SharedCollectionClient({ token }: { token: string }) {
  ...
  if (loading) {
    return <div className="max-w-[1200px] px-4 sm:px-6 md:px-[48px] py-12 mx-auto"><div className="h-[40px] w-2/3 bg-[var(--c-gray-100)] rounded animate-pulse mb-6" /><div className="grid grid-cols-2 sm:grid-cols-3 gap-4">{Array.from({ length: 6 }).map((_, i) => <div key={i} className="aspect-square bg-[var(--c-gray-100)] rounded animate-pulse" />)}</div></div>
  }
  ...
  const renderCard = (p: Project) => {
    ...
    return (
      <Link key={p.id} href={href} className="group block">
        ...
        {caption && (
          <p className="text-[11px] text-[var(--c-gray-400)] italic mt-1">{caption}</p>
        )}
      </Link>
    )
  }
  ...
  return (
    <div className="max-w-[1200px] px-4 sm:px-6 md:px-[48px] py-12 mx-auto">
      <div className="mb-10 flex items-start justify-between gap-4">
        ...
        {collection.subtitle && (
          <p className="text-[14px] text-[var(--c-gray-500)] mt-1">{collection.subtitle}</p>
        )}
